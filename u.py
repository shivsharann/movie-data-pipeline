...
def fetch_omdb_data(title):
    ...
    try:
        ...
        # Remove the year in parentheses: "Toy Story (1995)" -> "Toy Story"
        if "(" in clean_title:
            clean_title = clean_title.split("(")[0].strip()

        # Handle titles like "American President, The" -> "The American President"
        if "," in clean_title:
...
def transform_data(movies):
    ...
    # Process only first 20 movies for faster testing (remove .head(20) later)
